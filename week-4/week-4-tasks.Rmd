...
Read the 3 diamonds data sets and show the head of that.

`diamonds.txt`
```{r,echo=FALSE}
head(read.csv('diamonds.txt', sep = '#'))

```

`diamonds.csv`
```{r,echo=FALSE}
head(read.csv('diamonds.csv', sep = ';'))

```

`diamonds_with_extra_lines.txt`
```{r,echo=FALSE}
head(read.csv('diamonds_with_extra_lines.txt', sep = ';', skip = 1))
...
t <- data.table(nycflights13::flights)
t[t$dep_delay>1000,]
t[which(t$dep_delay>1000),]
```
...
saveRDS(df, 'df.rds')
library(nycflights13)
ifelse(flights$day<10, paste0('0', flights$day), flights$day )
ifelse(1:10<5, 'less', 'more')
