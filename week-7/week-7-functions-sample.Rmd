...
# Solution
check_divisibility <- function(number) {
  ...
  else("It is neither divisible by 2 nor by 3.")
}
...
# Solution
check_number_complex_2 <- function(number) {
  ifelse (number == 1, "It is 1.",
          ifelse(number == 0, "It is 0.",
                 ifelse(class(number) != "numeric", "It is not a number!", 
                        "It is neither 1 nor 0. But it is a number.")))
}


check_number_complex_2(1)
check_number_complex_2(0)
check_number_complex_2("Misi")
check_number_complex_2(10)
```
# For loops
...
dice <- function(side_number=6) {
  x <- c(1:side_number)
  print(paste0("The result is: ",sample(x,1)) )
}
...
# Solution
df_multiply <- function(input_df, column, multiply_by) {
  if (class(input_df[[column]]) == "numeric") {
    for (i in 1:nrow(input_df)){
      input_df[i, "numbers_multiplied"] <- input_df[i, column] * multiply_by
    }
  return(input_df)
  } else {
    print("This is not an integer type column")
  }
}

df_multiply(df, "numbers", 2)
df_multiply(df, "letters", 3)
